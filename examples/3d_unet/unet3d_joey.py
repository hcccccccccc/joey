...
def UB(input_size, kernel_size, cat_layer):
    ...
    cat = joey.cat(input_size=(input_size), layer=cat_layer)
    CB2 = CB(input_size=(input_size[0], input_size[1], input_size[2]*2, input_size[3]*2, input_size[4]*2), kernel_size=(kernel_size))
    CB3 = CB(input_size=(input_size[0], kernel_size[0], input_size[2]*2, input_size[3]*2, input_size[4]*2), kernel_size=(kernel_size[0],1,1,1))
...
def unet_joey(batch_size, in_channel, depth, height, weight, filter):
    ...
    DB_part = DB1 + DB2 + DB3 + DB4 + DB5
        ...
        # Upward Block
    UB1 = UB(input_size=(batch_size, filter*16, depth/16, height/16, weight/16), kernel_size=(filter*8,3,3,3), cat_layer=DB4[-1])
    # UB1_CB_CB = UB_CB_CB(input_size=(batch_size, filter*16, depth/8, height/8, weight/8), kernel_size=(filter*8,1,1,1))
    UB2 = UB(input_size=(batch_size, filter*8, depth/8, height/8, weight/8), kernel_size=(filter*4,3,3,3), cat_layer=DB3[-1])
    # UB2_CB_CB = UB_CB_CB(input_size=(batch_size, filter*8, depth/4, height/4, weight/4), kernel_size=(filter*4,1,1,1))
    UB3 = UB(input_size=(batch_size, filter*4, depth/4, height/4, weight/4), kernel_size=(filter*2,3,3,3),cat_layer=DB2[-1])
    # UB3_CB_CB = UB_CB_CB(input_size=(batch_size, filter*4, depth/2, height/2, weight/2), kernel_size=(filter*2,3,3,3))
    UB4 = UB(input_size=(batch_size, filter*2, depth/2, height/2, weight/2), kernel_size=(filter,3,3,3),cat_layer=DB1[-1])
